...
(defmethod enter (thing (container layered-container))
  (enter thing (aref (%objects container) (layer-index thing))))

(defmethod leave (thing (container layered-container))
  (leave thing (aref (%objects container) (layer-index thing))))

(defmethod predecessor (thing (container layered-container))
  (let ((layers (%objects container)))
    (let* ((layer (aref layers (layer-index thing)))
           (layer-idx (%object->index layer))
           (index (gethash thing layer-idx)))
      (cond ((null index))
            ((< 0 index)
             (aref layer (1- index)))
            (T
             (loop while (< 0 layer-idx)
                   do (let ((bag (%objects (aref layers (decf layer-idx)))))
                        (when (< 0 (size bag))
                          (return (aref (%objects bag) (1- (size bag))))))))))))
